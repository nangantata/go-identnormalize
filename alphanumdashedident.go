...
// AlphabetNumberDashOnlyIdentifier normalize given original identifier into identifier
// matches pattern `[a-zA-Z]([a-zA-Z0-9-]*)`.
// Characters out of acceptable range will be removed.
func AlphabetNumberDashOnlyIdentifier(originalIdent string, maxIdentifierLength int) string {
	...
	for _, ch := range originalIdent {
		...
		if (ch > unicode.MaxASCII) ||
			(!(unicode.IsLetter(ch) || unicode.IsDigit(ch) || (ch == '-'))) ||
...
// AlphabetNumberDashOnlyIdentifierPath normalize given originalIdentPath into normalizedIdentPath and
// normalizedFragments with each fragement matches pattern `[a-zA-Z]([a-zA-Z0-9-]*)`.
// Characters out of acceptable range will be removed.
func AlphabetNumberDashOnlyIdentifierPath(
